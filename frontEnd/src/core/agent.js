...
export const mainUrl = "https://panel.katinojob.ir/";
// export const mainUrl = "https://localhost:44377/";
export const baseUrl = mainUrl + "api/";
axios.defaults.baseURL = mainUrl + "api";
export const avatarUrl = mainUrl + "img/employeeAvatar";
...
axios.interceptors.response.use(undefined, async (error) => {
  ...
  if (error?.response?.status === 401 || error?.response?.status === 403) {
    ...
    window.location.href = "/Employee/Login?gotUnauthorizedError=true";
    // window.history.pushState({})

    // await history.push("/Employee/Login/");
...
const Account = {
  UpdateLastSeen: () => requests.post(`/Account/UpdateLastSeen`),
  SiteVisit: () => requests.post(`/Account/sitevisit`),
}
const Adver = {
...
const ReportAdvertS = {
  // plan: () => requests.get("/plan/getallplanforemployee"),
  create: (model) =>
    requests.post(`/ReportAdvertS/Create`, model),
};
...
const Refrences = {
  refrences: () => requests.get("/Account/GetAllAgent"),
